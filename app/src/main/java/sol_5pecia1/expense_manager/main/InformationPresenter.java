...
public class InformationPresenter implements MainContract.InformationListener {
    ...
    public void refreshView() {
        ...
        float weekdayPrecentage = getBudgetPercentage(
                preferenceModel.getWeekdayBudget()
                , currentSettlement
                , nextSettlement
        );
        float weekendPrecentage = getBudgetPercentage(
        ...
        Money budget = new Money((int)(
                monthBudget.getMoney() * weekdayPrecentage * leftWeekday
                +  monthBudget.getMoney() * weekendPrecentage * leftWeekend));
        Money planSpend = budget.minus(monthUsage);
        planSpend = planSpend.minus(new Money(10000));
        //.////////////////////
